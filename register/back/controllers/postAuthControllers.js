...
exports.postAuth = async (request, response) => {
    ...
    try {
        ...
        await database.pool.query(
            'INSERT INTO users (username, email, password, profile_image) VALUES ($1, $2, $3, $4)', [username, email, hashPassword, profileImagePath],
        )

        return response.status(201).json({ message: "회원가입이 완료되었습니다.", success: true });
    } catch (error) {
...
exports.postLogin = async (request, response) => {
    ...
    try {
        ...
        return response.status(500).json({ message: "로그인 오류 발생: " + error.message });
    }
}