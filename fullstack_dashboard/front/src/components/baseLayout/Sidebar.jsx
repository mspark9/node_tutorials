...
import { setSidebarClose, setSidebarOpen } from '../../redux/slices/sidebarSlice'
    ...
    return (
        <div className={`dark:bg-gray-950 bg-white py-5 px-4 dark:shadow-[0_0.125rem_0.25rem_rgba(255,255,255,0.3)] shadow-[0_0.125rem_0.25rem_rgba(165,163,174,0.3)] w-[20%] flex flex-col rounded-sm z-[1000] fixed h-full ${
            ...
            <div className='sidebar-top mb-[32px] flex items-center justify-between'>
                ...
                <button className='sidebar-close-btn text-black p=[0.125rem] rounded-sm bg-white cursor-pointer hover:bg-gray-300' onClick={() => dispatch(setSidebarOpen())}>
                    <MdOutlineClose />
                </button>
            </div>
                                ...
                                MENU_LISTS.map((menu, index) => (
                                    <li key={index} className='menu-item w-full'>
                                        <Link
                                            ...
                                            className={`h-[44px] flex items-center gap-x-[14px] py-0.5 px-5 font-medium w-full ${index === currentTab ? 'bg-blue-700 dark:text-white text-gray-950 rounded-sm' : ''}`}
                                            onClick={() => { selectMenuHandler(index) }}>
                                            <span
                                                className={`w-5 ${index === currentTab ? 'invert-[1] brightness-[100%]' : ''}`}>
                                                <img src={menu.icon} alt={menu.alt} />
                                            </span>
                                            <span className={`${index === currentTab ? 'text-white dark:text-white' : ''}`}>{menu.title}</span>
                                        </Link>
                                    </li>
                                ))
                            }
