                        ...
                        TARGET_REALITY_LISTS.map((item, index) => (
                            <div key={index} className='mt-4 flex items-center justify-between'>
                                <div className='flex items-center gap-x-2.5'>
                                    <div className={`w-10 h-10 rounded-sm flex place-content-center ${index === 0 ? 'bg-[#e2fff3]' : 'bg-[#fff4de]'}`}>
                                        <img src={item.icon} alt={item.title} className='w-6' />
                                    </div>
                                    <div>
                                        <h4 className='text-xs text-[#151d48] dark:text-gray-300'>{item.title}</h4>
                                        <p className='text-[10px] text-[#a5aea3] dark:text-gray-500'>{item.subtitle}</p>
                                    </div>
                                </div>
                                <div>
                                    <p className={`font-semibold dark:text-gray-300 ${index === 0
                                            ? "text-[#00e096]"
                                            : "text-[#ffa412]"
                                        }`}>{item.value}</p>
                                </div>
